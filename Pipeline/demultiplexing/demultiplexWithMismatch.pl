...
#### my $str   = "abcabcdefgabcdXfgabYdefgabcdZZg";
#### my $match = "abcdefg";
#### my $len = length($match);
###
#### for my $i (0..length($str)-$len) {
###    # my $s = substr $str, $i, $len;
###    # my $x = $s ^ $match;
###    # $x =~ tr/\0//d;  # equal characters xor to \0
###	# print length($x),"\n";
###    # print "'$s' at offset $i matched\n" unless length($x) > 1;
#### }

# my $str   = "xabcdgabcdXfgabYdefgabcdZZg";
# my $match = "abcdefg";
# my $len = length($match);
...
perl demultiplex.pl --barcodes <Barcode file> -1 <Mate1> -2 <Mate2> --suffix <suffix to add> --mismatch <>
